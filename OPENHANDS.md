...
# このプロパティは、OpenHands CLIが関連するドキュメントの更新を検知するために必要です。消去しないでください。
---

このファイルは、[OpenHands CLI](https://www.anthropic.com/OpenHands-code) がこのリポジトリのコードを扱う際のガイダンスを提供します。

## プロジェクト概要

このプロジェクトは、OpenHands CLIでの開発に最適化されたPythonプロジェクトテンプレートです。
厳格な型チェック、自動化されたコード品質管理、CIに加えて、GitHub CLIを使用したGitHub操作をサポートします。
また、OpenHands CLIとの協働をサポートするためのドキュメントも提供します。
...
├── template/                    # **重要**: OpenHands CLIのベストプラクティス・モデルコード
│   ├── src/
...
OpenHands CLIは `gh` コマンドを使用してGitHub操作を行うことができます。
...
make issue TITLE="OpenHands CLI改善" BODY="〜の部分で改善が必要です" LABEL="documentation"
...
## OpenHands.md自動更新トリガー

**重要**: 以下の状況でOpenHands.mdの更新を検討してください：
